 ...
 */
public class AmConllWithSourcesCreatorParallel {
    ...
    private AmConllWithSourcesCreatorParallel(SupertagDictionary supertagDictionary) {
    ...
    private static void createCorpus(List<SGraph> graphCorpus, List<DecompositionPackage> decompositionPackageList,
        ...
        AmConllWithSourcesCreatorParallel creator = new AmConllWithSourcesCreatorParallel(supertagDictionary);


        AtomicInteger fails = new AtomicInteger();
        AtomicInteger nondecomposeable = new AtomicInteger();
        List<AmConllSentence> amConllSentenceList = new ArrayList<>();
        List<Integer> range = IntStream.rangeClosed(0, graphCorpus.size() -1).boxed().collect(Collectors.toList());
        range.parallelStream().forEach((Integer i) -> {
            ...
            try {
                ...
                nondecomposeable.getAndIncrement();
            } catch (java.lang.Exception ex) {
                System.err.println(i);
//                    System.err.println(graph.toIsiAmrStringWithSources());
                ex.printStackTrace();
