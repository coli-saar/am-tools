...
public class EDSConverter {
    ...
    public static MRInstance toSGraph(SGraph eds, String sent){
        ...
        MRInstance inst = Aligner.extractAlignment(eds, sent);
        
        inst.setGraph(encodeLnks(inst.getGraph()));
    ...
    public static SGraph evaluateDependencyTree(ConllSentence sent) throws AMDependencyTree.ConllParserException, ParserException, ParseException{
        ...
        List<Pair<Integer,Integer>> stanfSpans = (List<Pair<Integer,Integer>>) p.left;
        ...
        SGraph evaluatedGraph = restoreComplexSpans(evaluated);
...
//        System.err.println(EDSUtils.simpleAlignViz(inst));
    ...
    public static SGraph ensureRootIsWritable(SGraph sg){
        SGraph copy;
        if (!sg.getNodeForSource("root").startsWith("_")){
            copy = sg.merge(new SGraph());
        } else {
     ...
     * @return 
     */
    public static SGraph makeNodeNamesExplicit(SGraph eds){
        SGraph copy = new SGraph();
     ...
     * @return 
     */
    public static SGraph undoExplicitAnon(SGraph eds){
        SGraph copy = new SGraph();
        for (String n : eds.getAllNodeNames()){
            String name = n;
    ...
    private static boolean hideNodeName(String node, SGraph eds){
        return eds.getSourcesAtNode(node).isEmpty() && eds.getGraph().inDegreeOf(eds.getNode(node)) > 0 && eds.getGraph().incomingEdgesOf(eds.getNode(node)).stream().allMatch(edge -> edge.getLabel().equals("lnk") || edge.getLabel().equals("carg"));
    }
     ...
     * @return 
     */
    public static SGraph encodeLnks(SGraph eds){
        SGraph copy = eds.merge(new SGraph());
        Set<String> complexSpans = EDSUtils.getComplexSpans(eds).stream().map(pair -> "<"+pair.left+":"+pair.right+">").collect(Collectors.toSet());
        List<String> deleteMe = new ArrayList<>();
        for (GraphNode n : copy.getGraph().vertexSet()){
            Matcher m = Aligner.lnk.matcher(n.getLabel());
            if (m.matches()){
     ...
     * @param als 
     */
    public static void restoreSimpleSpans(SGraph sg, List<String> words,List<Pair<Integer, Integer>> stanfSpans, List<Alignment> als){
        for (Alignment a : als){
            for (String nodeName : a.nodes){
                if (sg.getNode(nodeName).getLabel().equals(SIMPLE_SPAN)){
                    ...
                    if (nodeThatSpansSeveralTokens.isPresent()){
                        ...
                        Pair<Integer,Integer> charSpanL = stanfSpans.get(l);
                        Pair<Integer,Integer> charSpanR = stanfSpans.get(r);
                        sg.getNode(nodeName).setLabel("<"+charSpanL.left+":"+charSpanR.right+">");   
                    } else {
                        Span mySpan = a.span; //token span
                        assert mySpan.isSingleton();
                        Pair<Integer,Integer> charSpan = stanfSpans.get(mySpan.start);
                        sg.getNode(nodeName).setLabel("<"+charSpan.left+":"+charSpan.right+">");   
                    }
                }
            }
        }
     ...
     */
    @Deprecated
    public static SGraph restoreComplexSpans(Tree<Set<String>> treeWithNodeNames, SGraph eds,List<Pair<Integer,Integer>> stanfSpans, List<Alignment> als) {
        SGraph copy = eds.merge(new SGraph());
     ...
     * @return 
     */
    public static SGraph restoreComplexSpans(SGraph eds){
        SGraph copy = eds.merge(new SGraph());
        ...
        HashMap<String,Pair<Integer,Integer>> node2span =  new HashMap(); //maps a node (name) to the character span it covers
        
        //first add all simple spans
        for (GraphNode node : copy.getGraph().vertexSet()){
            Optional<GraphEdge> perhapsLnkEdge =  g.outgoingEdgesOf(node).stream().filter(e -> e.getLabel().equals("lnk")).findFirst();
            if (perhapsLnkEdge.isPresent()) {
                //parse the label into an Int,Int pair and store it.
                String spanString = perhapsLnkEdge.get().getTarget().getLabel();
                Matcher m = lnk.matcher(spanString);
                if (m.matches()){
                    Pair<Integer,Integer> span = new Pair<>(Integer.parseInt(m.group(1)),Integer.parseInt(m.group(2)));
                    node2span.put(node.getName(), span);
                }
            }
        }
        ...
        for (GraphNode node : topo){
            if (!g.incomingEdgesOf(node).stream().anyMatch(e -> e.getLabel().equals("carg"))) { //carg nodes don't get spans
                if (g.outDegreeOf(node) > 0) {
                    GraphEdge outgoingLnkEdge = g.outgoingEdgesOf(node).stream().filter(e -> e.getLabel().equals("lnk")).findFirst().get();
                    if (g.outgoingEdgesOf(node).stream().anyMatch(e -> e.getTarget().getLabel().equals(COMPLEX_SPAN))) {
                        ...
                        int min;
                        int max;
                        if (g.outDegreeOf(node) < 2 || g.outgoingEdgesOf(node).stream().filter(e -> !e.getLabel().equals("lnk")).anyMatch(e -> !node2span.containsKey(e.getTarget().getName()))) { 
                            ...
                            min = g.outgoingEdgesOf(node).stream().filter(e -> !e.getLabel().equals("lnk")).map(e -> node2span.get(e.getTarget().getName()).left).min(Integer::compare).get();
                            max = g.outgoingEdgesOf(node).stream().filter(e -> !e.getLabel().equals("lnk")).map(e -> node2span.get(e.getTarget().getName()).right).max(Integer::compare).get();
                        }
                        node2span.put(node.getName(), new Pair<>(min,max));
                        outgoingLnkEdge.getTarget().setLabel("<"+min+":"+max+">");
                    }
            }
            }
     ...
     * @return 
     */
    public static String toEDM(SGraph eds){
        StringBuilder b = new StringBuilder();
        HashMap<String,Pair<Integer,Integer>> n2s = EDSUtils.nodeToSpan(eds);
        for (GraphNode n : eds.getGraph().vertexSet()){
            ...
            } else if (eds.getGraph().incomingEdgesOf(n).stream().allMatch(edge -> !edge.getLabel().equals("lnk"))) {
        ...
        if (n2s.containsKey(root)){
            ...
            Pair<Integer,Integer> p = (Pair<Integer,Integer>) n2s.get(root);
            b.append(p.left);
            b.append(":");
            b.append(p.right);
            b.append(" ; ");
        ...
        for (GraphEdge e : eds.getGraph().edgeSet()){
            if (!e.getLabel().equals("lnk") && !e.getLabel().equals("carg"))
    ...
    private static void addInfo(StringBuilder b, HashMap<String,Pair<Integer,Integer>> n2s, String nodeName, String rel, String rel2){
    if (n2s.containsKey(nodeName)){
            Pair<Integer,Integer> p = (Pair<Integer,Integer>) n2s.get(nodeName);
            b.append(p.left);
            b.append(":");
            b.append(p.right);
            b.append(" ");
    ...
    private static void addEdgeInfo(StringBuilder b, HashMap<String,Pair<Integer,Integer>> n2s, String fromNode, String toNode, String label){
    if (n2s.containsKey(fromNode) && n2s.containsKey(toNode)){
            Pair<Integer,Integer> p = (Pair<Integer,Integer>) n2s.get(fromNode);
            b.append(p.left);
            b.append(":");
            b.append(p.right);
            b.append(" ");
            ...
            b.append(" ");
            p = (Pair<Integer,Integer>) n2s.get(toNode);
            b.append(p.left);
            b.append(":");
            b.append(p.right);
            b.append(" ; ");
